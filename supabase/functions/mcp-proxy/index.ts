...
async function handleToolCall(name: string, args: Record<string, unknown>): Promise<unknown> {
  switch (name) {
    ...
    case 'send_email': {
      ...
      // Controlled by the building TOGGLE only. Fail CLOSED: send strictly when
      // collection_notifications_enabled is EXPLICITLY true. Missing building /
      // lookup error / disabled → blocked. No email goes out unless the toggle
      // is on — flipping the toggle is the single control (no code change).
      let enabled = false
      if (building_id) {
      ...
      if (!enabled) {
        await supabase.from('notification_log').insert({
          ...
          body: html,
          status: 'blocked',
          error_message: 'collection_notifications_disabled (toggle off)',
        })
