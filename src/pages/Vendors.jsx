...
function StarRating({ rating, interactive = false, onChange }) {
  ...
  for (let i = 1; i <= 5; i++) {
    stars.push(
      <span
        key={i}
        className={`${i <= rating ? 'text-amber-400' : 'text-[var(--text-muted)]'} ${interactive ? 'cursor-pointer hover:scale-110 transition-transform' : ''}`}
  ...
  return <span className="text-base">{stars}</span>
}
...
function VendorCard({ vendor, stats, onClick, compareMode, isSelected, onToggleCompare }) {
  ...
  return (
    <Card className="cursor-pointer relative" onClick={() => !compareMode && onClick?.()}>
      <CardContent className="pt-5">
        ...
        {tags.length > 0 && (
          <div className="flex flex-wrap gap-1 mb-2">
            {tags.map((tag) => (
              <Badge key={tag.label} variant={tag.variant} className="text-[10px] px-1.5 py-0">
                {tag.label}
              </Badge>
            ))}
          </div>
        ...
        {/* Stats row */}
        <div className="flex items-center gap-3 mt-3 pt-3 border-t border-[var(--border)] text-xs text-[var(--text-secondary)]">
  ...
  return (
    <Card>
      <CardContent className="pt-5">
        ...
        <p className="text-xs text-[var(--text-secondary)] mt-1 mb-2">{vendor.description}</p>
