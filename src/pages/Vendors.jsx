...
function buildInviteMessage(vendor) {
  ...
  return `שלום${vendor?.name ? ' ' + vendor.name : ''},\n\n` +
    `אנחנו *ועד פלוס* — מערכת לניהול תחזוקת בנייני מגורים. אנו מנהלים עבור ועדי בתים את הטיפול בתקלות ובעבודות, ומפנים אותן ישירות למאגר ספקים מומלצים.\n\n` +
    `נשמח לצרף אתכם${trade} למאגר. המשמעות עבורכם: זרם קבוע של פניות לעבודות מבניינים באזורכם — ישירות לנייד, עם כל פרטי התקלה (כולל תמונה) מראש.\n\n` +
    `*כדי להצטרף, אנא אשרו שאתם מעוניינים לקבל פניות לעבודות.* רק לאחר אישורכם תתחילו לקבל קריאות מהמערכת.\n\n` +
    `לאישור — השיבו "מאשר" להודעה זו. תודה, ונשמח לשתף פעולה!\n` +
    `ועד פלוס`
}
