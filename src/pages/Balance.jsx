  ...
  const monthlyData = useMemo(() => {
    ...
    return HEBREW_MONTHS.map(({ value: month, label }) => {
      ...
      const bankIncome = allTx
        .filter(tx => tx.building_id === selectedBuilding.id && tx.month === monthKey && Number(tx.credit) > 0)
        .reduce((s, tx) => s + (Number(tx.credit) || 0), 0)

      // Bank expenses: debits
      const bankExpenses = allTx
        .filter(tx => tx.building_id === selectedBuilding.id && tx.month === monthKey && Number(tx.debit) > 0)
